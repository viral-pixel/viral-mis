  ...
  return (
    <div>
      <SectionHead
        ...
        sub="Management view — replicates the three report sheets, derived from Ketan's vegetable purchase data"
      />
...
function MonthlySummaryTab() {
  ...
  // Count L/D entry (inline edit + Excel import) is Admin-only — the API
  // enforces it too; this just keeps view-only users from seeing controls
  // that would fail.
  const [isAdmin, setIsAdmin] = useState(false);
  useEffect(() => { fetch("/api/auth/me").then((r) => r.json()).then((d) => setIsAdmin(!!d.user?.isAdmin)); }, []);
  ...
  const load = () => fetch("/api/vegetable-analysis/summary").then((r) => r.json()).then(setRows);
  useEffect(() => { load(); }, []);
  ...
  const saveCount = async (monthKey: string) => {
    ...
    setSaving(true);
    const res = await fetch("/api/vegetable-analysis/meal-count", {
  ...
  return (
    <div>
      ...
      <ChartPanel
        title="Monthly Summary" sub={isAdmin ? "Count L/D is the only figure you enter — everything else comes from Ketan's data automatically" : "Calculated automatically from Ketan's purchase data and the monthly Count L/D"}
        action={
          <div style={{ display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
            ...
            <a href={`/api/vegetable-analysis/export-summary?fields=${[...fields].join(",")}${from ? `&from=${from}` : ""}${to ? `&to=${to}` : ""}`} style={{ textDecoration: "none" }}>
              <Btn variant="ghost">Export Excel</Btn>
            </a>
            {isAdmin && <label style={{ display: "inline-flex" }}>
              ...
              <input
                ...
                onChange={async (e) => {
                  ...
                  fd.append("file", file);
                  const res = await fetch("/api/vegetable-analysis/meal-count/import", { method: "POST", body: fd });
                  ...
                  <tr key={r.monthKey}>
                    ...
                    {fields.has("perplate") && (
                      <>
                        <Td>
                          {!isAdmin ? (
                            r.countLD != null ? r.countLD.toLocaleString("en-IN") : "—"
...
function ItemWiseTab() {
  ...
  useEffect(() => {
    fetch("/api/vegetable-analysis/summary").then((r) => r.json()).then((rows: MonthlyRow[]) => {
  ...
  useEffect(() => {
    if (!month) return;
    setData(null);
    fetch(`/api/vegetable-analysis/item-wise?month=${month}`).then((r) => r.json()).then(setData);
  }, [month]);
    ...
    <ChartPanel
      ...
      action={
        <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
          ...
          {month && (
            <a href={`/api/vegetable-analysis/export-item-wise?month=${month}`} style={{ textDecoration: "none" }}>
              <Btn variant="ghost">Export Excel</Btn>
            </a>
          )}
        </div>
...
function PotatoOnionTab() {
  ...
  useEffect(() => {
    fetch("/api/vegetable-analysis/potato-onion-summary").then((r) => r.json()).then(setRows);
  }, []);
  ...
  return (
    <ChartPanel
      ...
      action={
        <a href="/api/vegetable-analysis/export-potato-onion" style={{ textDecoration: "none" }}>
          <Btn variant="ghost">Export Excel</Btn>
        </a>
      }
    >
