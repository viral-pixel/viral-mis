...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await prisma.rotiCategory.delete({ where: { id: Number(id) } });
    return NextResponse.json({ ok: true });
  } catch {
    return NextResponse.json({ error: "This category has entries recorded against it and can't be deleted." }, { status: 409 });
  }
}
