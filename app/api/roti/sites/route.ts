...
export async function GET() {
  ...
  const sites = await prisma.rotiSite.findMany({ orderBy: [{ sortOrder: "asc" }, { name: "asc" }] });
...
export async function POST(req: NextRequest) {
  ...
  const max = await prisma.rotiSite.aggregate({ _max: { sortOrder: true } });
  try {
    const created = await prisma.rotiSite.create({
