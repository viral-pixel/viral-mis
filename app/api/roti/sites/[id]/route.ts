...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await prisma.rotiSite.delete({ where: { id: Number(id) } });
    return NextResponse.json({ ok: true });
  } catch {
    return NextResponse.json({ error: "This site has entries recorded against it and can't be deleted." }, { status: 409 });
  }
}
